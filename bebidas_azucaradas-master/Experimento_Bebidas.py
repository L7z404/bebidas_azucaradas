...
path =r'C:\Users\leone\cosas\bebidas_azucaradas-master\bebidass'
list_of_images = []
...
thisExp = data.ExperimentHandler(name=expName, version='',
    ...
    originPath='C:\\Users\\leone\\Desktop\\bebidas_azucaradas\\experimeno_bebidas_lastrun.py',
...
img_aleatorias = data.TrialHandler(nReps=veces_rep, method='random', 
    extraInfo=expInfo, originPath=-1,
    trialList=[None],
    seed=None, name='img_aleatorias')
thisExp.addLoop(img_aleatorias)  # add the loop to the experiment
thisImg_aleatoria = img_aleatorias.trialList[0]  # so we can initialise stimuli with some values
# abbreviate parameter names if possible (e.g. rgb = thisImg_aleatoria.rgb)
if thisImg_aleatoria != None:
    for paramName in thisImg_aleatoria:
        exec('{} = thisImg_aleatoria[paramName]'.format(paramName))

for thisImg_aleatoria in img_aleatorias:
    currentLoop = img_aleatorias
    # abbreviate parameter names if possible (e.g. rgb = thisImg_aleatoria.rgb)
    if thisImg_aleatoria != None:
        for paramName in thisImg_aleatoria:
            exec('{} = thisImg_aleatoria[paramName]'.format(paramName))
    ...
    while continueRoutine:
        ...
        if cruz.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
            ...
            outlet.push_sample(markers['cruz'])
        ...
        if imagen.status == STARTED:
            ...
            if tThisFlipGlobal > imagen.tStartRefresh + 2.0-frameTolerance:
                ...
                #outlet.push_sample(markers['img'])
                
                if "AguaBonafont" in imgg:
                    print("encontrado")

                if "AguaMineralizadaTopoChico" in imgg:
                    print("encontrado")

                if "Aguafrescadefrutas" in imgg:
                    print("encontrado")

                if "Atolecasero" in imgg:
                    print("encontrado")

                if "Bebidasdeportivas" in imgg:
                    print("encontrado")

                if "CafeSinAzucar" in imgg:
                    print("encontrado")

                if "CervezaLight" in imgg:
                    print("encontrado")

                if "cocacol" in imgg:
                    print("encontrado")

                if "Delawarepunch" in imgg:
                    print("encontrado")

                if "JugodeNaranjaNatural" in imgg:
                    print("encontrado")

                if "JugodeZanahoriaNatural" in imgg:
                    print("encontrado")

                if "JugoJumexPina" in imgg:
                    print("encontrado")

                if "JumezUnicoFresco" in imgg:
                    print("encontrado")

                if "LecheAlpuraSemidescremada" in imgg:
                    print("encontrado")

                if "LecheDeSoya" in imgg:
                    print("encontrado")

                if "Lecheentera" in imgg:
                    print("encontrado")

                if "LecheHersheyChocolate" in imgg:
                    print("encontrado")

                if "LecheLalaLigth" in imgg:
                    print("encontrado")
                    
                if "LecheLalaSemidescremada" in imgg:
                    print("encontrado")

                if "LecheNesquikChocolate" in imgg:
                    print("encontrado")

                if "LecheYomiLala" in imgg:
                    print("encontrado")

                if "LicuadodeFrutasconLeche" in imgg:
                    print("encontrado")

                if "Maizena" in imgg:
                    print("encontrado")

                if "manzanalift" in imgg:
                    print("encontrado")

                if "manzanitasol" in imgg:
                    print("encontrado")

                if "Miel" in imgg:
                    print("encontrado")

                if "MinuteMaidNaranja" in imgg:
                    print("encontrado")

                if "mirinda" in imgg:
                    print("encontrado")

                if "NesteaLimon" in imgg:
                    print("encontrado")
                ...
                if "TéSinAzucar" in imgg:
                    print("encontrado")

                if "VinoBlanco" in imgg:
                    print("encontrado")

                if "Vinotinto" in imgg:
                    print("encontrado")
    ...
    for thisComponent in mostrar_imagenComponents:
        if hasattr(thisComponent, "setAutoDraw"):
            thisComponent.setAutoDraw(False)
    img_aleatorias.addData('cruz.started', cruz.tStartRefresh)
    img_aleatorias.addData('cruz.stopped', cruz.tStopRefresh)
    # check responses
    if b_siguiente.keys in ['', [], None]:  # No response was made
        b_siguiente.keys = None
    img_aleatorias.addData('b_siguiente.keys',b_siguiente.keys)
    if b_siguiente.keys != None:  # we had a response
        img_aleatorias.addData('b_siguiente.rt', b_siguiente.rt)
    img_aleatorias.addData('b_siguiente.started', b_siguiente.tStartRefresh)
    img_aleatorias.addData('b_siguiente.stopped', b_siguiente.tStopRefresh)
    img_aleatorias.addData('imagen.started', imagen.tStartRefresh)
    img_aleatorias.addData('imagen.stopped', imagen.tStopRefresh)
    # the Routine "mostrar_imagen" was not non-slip safe, so reset the non-slip timer
    routineTimer.reset()
# completed veces_rep repeats of 'img_aleatorias'
...
# set up handler to look after randomisation of conditions etc
respuestas = data.TrialHandler(nReps=veces_rep, method='random', 
    extraInfo=expInfo, originPath=-1,
    trialList=[None],
...
for thisRespuesta in respuestas:
    ...
    while continueRoutine:
        ...
        if mouse.status == STARTED:  # only update if started and not finished!
            ...
            if mouse.isPressedIn(tr1):
                outlet.push_sample(markers['uno'])
            if mouse.isPressedIn(tr2):
                outlet.push_sample(markers['dos'])
            if mouse.isPressedIn(tr3):
                outlet.push_sample(markers['tres'])
            if mouse.isPressedIn(tr4):
                outlet.push_sample(markers['cuatro'])
            if mouse.isPressedIn(tr5):
                outlet.push_sample(markers['cinco'])
            if mouse.isPressedIn(tr6):
                outlet.push_sample(markers['seis'])
            if mouse.isPressedIn(tr7):
                outlet.push_sample(markers['siete'])
            if mouse.isPressedIn(tr8):
                outlet.push_sample(markers['ocho'])
            if mouse.isPressedIn(tr9):
                outlet.push_sample(markers['nueve'])
        ...
        if mouse_2.status == STARTED:  # only update if started and not finished!
            ...
            if buttons != prevButtonState:  # button state changed?
                ...
                if sum(buttons) > 0:  # state changed to a new click
                    ...
                    if gotValidClick:  
                        continueRoutine = False  # abort routine on response
            if mouse_2.isPressedIn(tr1):
                outlet.push_sample(markers['uno'])
            if mouse_2.isPressedIn(tr2):
                outlet.push_sample(markers['dos'])
            if mouse_2.isPressedIn(tr3):
                outlet.push_sample(markers['tres'])
            if mouse_2.isPressedIn(tr4):
                outlet.push_sample(markers['cuatro'])
            if mouse_2.isPressedIn(tr5):
                outlet.push_sample(markers['cinco'])
            if mouse_2.isPressedIn(tr6):
                outlet.push_sample(markers['seis'])
            if mouse_2.isPressedIn(tr7):
                outlet.push_sample(markers['siete'])
            if mouse_2.isPressedIn(tr8):
                outlet.push_sample(markers['ocho'])
            if mouse_2.isPressedIn(tr9):
                outlet.push_sample(markers['nueve'])
        ...
        if mouse_3.status == STARTED:  # only update if started and not finished!
            ...
            if buttons != prevButtonState:  # button state changed?
                ...
                if sum(buttons) > 0:  # state changed to a new click
                    ...
                    if gotValidClick:  
                        continueRoutine = False  # abort routine on response
            if mouse_3.isPressedIn(tr1):
                outlet.push_sample(markers['uno'])
            if mouse_3.isPressedIn(tr2):
                outlet.push_sample(markers['dos'])
            if mouse_3.isPressedIn(tr3):
                outlet.push_sample(markers['tres'])
            if mouse_3.isPressedIn(tr4):
                outlet.push_sample(markers['cuatro'])
            if mouse_3.isPressedIn(tr5):
                outlet.push_sample(markers['cinco'])
            if mouse_3.isPressedIn(tr6):
                outlet.push_sample(markers['seis'])
            if mouse_3.isPressedIn(tr7):
                outlet.push_sample(markers['siete'])
            if mouse_3.isPressedIn(tr8):
                outlet.push_sample(markers['ocho'])
            if mouse_3.isPressedIn(tr9):
                outlet.push_sample(markers['nueve'])
    ...
    routineTimer.reset()
# completed veces_rep repeats of 'respuestas'
